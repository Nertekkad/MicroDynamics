...
bacter_ml<-ctr(bacter_ml, "degree")

# Degree´s node-color matrix
colmat<-matrix(c(V(bacter_ml[[1]])$hl,V(bacter_ml[[2]])$hl,V(bacter_ml[[3]])$hl),
               nrow = length(V(bacter_ml[[1]])), ncol = length(bacter_ml))

absmat<-matrix(c(log(colSums(severe_data)+2)/20,log(colSums(moderate_data)+2)/20,
                 log(colSums(mild_data)+2)/20), nrow = length(V(bacter_ml[[1]])),
                 ncol = length(bacter_ml))
