...
ggplot(df, aes(x = x, y = y)) + geom_line(data = df) +
  labs(x = "r", y = "x_t",
...
Tomando la función simplificada, podemos entonces inferir que $x(0)=1/(c^{-c_{3}}+1/k)=x_{0}$. Para obtener el valor de la constante $c_{3}$, despejamos en la expresión anterior:

$$x_{0}(e^{-c_{3}}+\frac{1}{k})=1 \quad \rightarrow \quad x_{0}e^{-c_{3}}=1-\frac{x_{0}}{k} \quad \rightarrow \quad e^{-c_{3}}=\frac{(1-\frac{x_{0}}{k})}{x_{0}}$$
Por lo cual, obtenemos que $c_{3}$ se define como:

$$c_{3}=-ln(\frac{k-x_{0}}{x_{0}k})$$
