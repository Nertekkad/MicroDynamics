...
```{r}
# Function to generate bifurcation diagram for logistic map
logistic_bifurcation <- function(r, x0, n, m) {
  x <- numeric(n)
  x[1] <- x0
  for (i in 2:n) {
    x[i] <- r * x[i - 1] * (1 - x[i - 1])
  ...
  x_m <- x[(n - m):n]
  r_vals <- rep(r, m + 1)
  return(data.frame(r = r_vals, x = x_m))
}
# Parameters
r_values <- seq(1, 4, by = 0.01)
x0 <- 0.5
n <- 10000
m <- 100
# Generate bifurcation data
bifurcation_data <- data.frame(r = numeric(0), x = numeric(0))
for (r in r_values) {
  bif_data <- logistic_bifurcation(r, x0, n, m)
  bifurcation_data <- rbind(bifurcation_data, bif_data)
}
# Plot bifurcation diagram
library(ggplot2)
ggplot(bifurcation_data, aes(x = r, y = x)) +
...
Si ahora asumimos que la tasa de crecimiento *no* es constante, sino que varía en el tiempo $r(t)$, por lo que la tasa de crecimiento aquí está gobernada por las interacciones enterespecíficas.
...
Esta razón está relacionada con el llamado promedio relativo del **fitness** ($\kappa_{i}$) asociado a cada especie.
...
Si ahora asumimos que las interacciones son equivalentes: $\alpha_{ij}=\alpha_{ji}$, por lo tanto $\sigma^{R}_{ij}=\sigma^{R}_{ji}$.
...
Notemos que realmente no importa si la función está formulada con respecto a $x_{A}$ o a $x_{B}$, pues ambas son simétricas.
...
Si ahora sustituimos en $x^{*}(1-x^{*})(f_{A}'(x^{*})-f_{B}'(x^{*}))$.
...
Al poner atención a los escenarios previos, podemos entender que los puntos de equilibrio $x_{1}^{*}=0$ y $x_{2}^{*}=1$ implican escenarios de exclusión competitiva donde $(H_{ij}, H_{ji})$ comprendería $(0,1)$ y $(1,0)$, respectivamente. Por lo cual, $x_{1}^{*}<x_{2}^{*}<x_{3}^{*}$ donde los extremos sean estables o inestables, por lo tanto, $x_{3}^{*}$ sea inestable o estable, respectivamente. En cualquier escenario, $x_{3}^{*}$ estaría representando un escenario de coexistencia entre ambas especies.
