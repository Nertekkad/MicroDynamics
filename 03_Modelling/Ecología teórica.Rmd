...
Existen múltiples formas de clasificar ecuaciones diferenciales, iniciando por hacer una distinciójn entre aquellas que son continuas, donde $x(t)$ puede adoptar cualquier valor $\mathbb{R}$ en el tiempo $(-\infty, \infty)$. Por su parte, las funciones discretas solo pueden adoptar valores enteros $\mathbb{Z}$ en el tiempo, de modo que cada valor de $x_{t}$ depende del resultado obtenido previamente en $x_{t-1}$. En los modelos continuos, por lo tanto, es posible calcular los límites de la función y definir sus derivadas.
...
Las funciones también pueden ser autónomas o no-autónomas, siendo las primeras aquellas donde no existe una dependencia temporal. Un ejemplo clásico de este caso consiste en una masa que oscila en un resorte. En un sistema autónomo, esta oscilación disminuiría en función del tiempo hasta detenerse una vez el sistema haya discipado su energía cinética en forma de calor. Sin embargo, en un sistema no-autónomo se podría agregar un impulso periódico que introdusca energía cinética al sistema cada cierto tiempo.

Las funciones pueden ser deterministas o estocásticas, donde las primeras devuelven siempre el mismo conjunto de valores como resultado dados determinados valores de entrada, mientras que las funciones estocásticas no, dado su caracter aleatorio.

Llegados a este punto, podemos definir que una función de tipo:

$$\dot{x}=\frac{dx(t)}{dt}=f(x(t))$$

Es por lo tanto contínua, ordinaria, autónoma y determinista, donde $x(0)=x_{0}$ en $t=0$, siendo $x_{0}$ las condiciones iniciales del sistema.
...
La sensibilidad a las condiciones iniciales o caos, es la tendencia de un sistema a tomar un conjunto de valores radicalmente distintos con respecto a si mismo tras haber introducido condiciones iniciales similares. Esto no debe confundirse con la estocasticidad, dado que un sistema caótico es totalmente determinista, sin embargo, cualquier variación sobre las condiciones iniciales, por minúscula que sea, se amplifica en el tiempo dando como resultado una eventual divergencia de los resultados. Un sistema estocástico es, por definición, impredecible. El caos no ha sido reportado en sistemas contínuos unidimencionales, sin embargo, si está presente en funciones discretas, como sucede con el modelo discreto de Verhulst:
...
En modelos continuos, el caos solo aparece en sistemas de dimención igual o mayor a tres, como sucede con el modelo de convección atmosférica de Lorenz, mismo que dio nombre al atractor de Lorenz con su característica forma de mariposa. Este modelo, de hecho, abrió las puertas al estudio de los sistemas caóticos y su intrigante naturaleza.
...
Para ejemplificar lo anterior, podemos emplear la siguente función que describe el **efecto Allee**, originalmente empleada para modelar poblaciones animales que requieren superar un valor umbral
mínimo para poder asegurar la supervivencia de la población. De encontrarse dicha población por debajo del umbral, esta decae hasta la extición.
...
Al evaluar $x_{1}^{*}=0$, la $df/dx=r(-\alpha)=-\alpha{x}<0$, y por lo tanto, estable. En $x_{2}^{*}=k$, obtenemos que $df/dx=rk(-1/k)(k-\alpha)=-r(k-\alpha)<0$, que es estable, tomando en consideración que $k>\alpha$. Para $x_{3}^{*}=\alpha$, obtenemos que $df/dx=r\alpha(1-\alpha/k)>0$, y por ende, inestable.
...
Analicemos el comportamiento del punto cerca de

## Modelos ecológicos unidimencionales {-}
...
Por lo tanto, para mantener la equivalencia, $B=A/k$, lo que da como resultado que $A\alpha=1$, ergo $A=1/\alpha$. Si ahora sustituímos $A$ en $B$, obtenemos que $B=1/(\alpha{k})$. A raíz de lo anterior, obtenemos que:
...
Para resolver la segunda parte de la integral, realizamos una integración por cambio de variable, demodo que obtenemos que $u=\alpha-x/k$ y $du=-dx/k$. Despejando esta última parte, obtenemos que $dx=-kdu$. Por lo tanto:
...
Ahora sustituímos $x_{i}$ en la ecuación de $x_{j}$:
...
Ahora sustituímos $x_{j}^{*}$ en la ecuación $x_{i}=1/\alpha_{ii}-(\alpha_{ij}/\alpha_{ii})x_{j}$:
...
El hecho de que ambos sistemas sean análogos entre si, se puede entender conceptualmente como que el LVg considera interaccipnes directas, mientras que en el MacArthur, estas interacciones emergen indirectamente de la competencia por recursos entre poblaciones.

Para que la especie $i$ pueda coexistir con el resto de especies, la tasa de cambio debe ser positiva, es decir, la población no debe decrecer ($r_{i}(t)>0$).
...
Concideremos un escenario con dos especies competidoras. Al tratarse de un modelo discreto, imaginemos una retícula de $nxn$ dimenciones, cuyas casillas pueden o no estar ocupadas por miembros de la especie $i$ o de la especie $j$. Si un miembro de alguna de las dos especies muere, deja por lo tanto, una casilla vacía por la que ambas especies deben competir por ocupar.
